...
export class HtmlStrategy extends BaseTranslationStrategy {
  ...
  constructor(config?: HtmlConfig) {
    ...
    this.skipTags = config?.skipTags || ['script', 'style', 'code', 'pre', 'svg', 'math'];
  }
  ...
  private extractEncoding(document: any): string {
    ...
    const metaCharset = findNode(
      document,
      (node) => node.nodeName === 'meta' && node.attrs?.some((attr: any) => attr.name === 'charset')
    );
