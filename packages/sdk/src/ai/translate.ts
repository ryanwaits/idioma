...
import { getDefaultModelForProvider, getEffectiveProviderAndModel } from './defaults';
...
export async function translateText(
  ...
  const _actualProvider = effectiveProvider;
  ...
  const looksLikeFrontmatter =
    trimmedText.includes('title:') || trimmedText.includes('description:');

  // Format preserved terms for the prompt
  const preserveInstructions =
    preservedTerms && preservedTerms.length > 0
