...
export class FormatDetector {
  ...
  private async registerDefaultStrategies(): Promise<void> {
    ...
    const strategyModules = [
      ...
      { extensions: ['.js', '.ts', '.jsx', '.tsx', '.mjs', '.cjs'], module: '../strategies/javascript.js' }
    ];

    for (const { extensions, module } of strategyModules) {
      try {
        const strategyModule = await import(module);
        const StrategyClass = strategyModule.default || strategyModule[Object.keys(strategyModule)[0]];
        ...
        // console.debug(`Strategy ${module} not yet implemented`);
      }
    }
  }
